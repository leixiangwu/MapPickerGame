...
public class AccountsImporterExporter {
    ...
    public static final String ACCOUNTS_DATA_PATH = "./data/accounts/";
        ...
        for (int i = 0; i < listOfFiles.length; i++) {
            if (listOfFiles[i].isFile()) {
                ...
                if (files.endsWith(".txt") || files.endsWith(".TXT")) {
                    loadAccountsIntoManager(listOfFiles[i], manager);
                }
            }
        }
    }

    private void loadAccountsIntoManager(File fileToLoad, AccountManager manager) {
    ...
    public void saveAccounts(AccountManager manager) {
        ...
        for (Account account : manager.getAllAccounts()) {
            ...
            try {
                ...
                writer.write(account.getAccountName() + SPLIT_SIGN + account.getRegionPlayedSize() + GO_NEXT_LINE);
    ...
    private void writeAccountInfoHelper(BufferedWriter writer, Account account) {
        ...
        while (iterator.hasNext()) {
            ...
            try {
                writer.write(regionName + SPLIT_SIGN + account.getNumModesPlayed(regionName) + GO_NEXT_LINE);
            } catch (IOException ex) {
    ...
    private void writerModeRegionStats(BufferedWriter writer, String mode, RegionStats stats) {
        String stringToWrite = mode + SPLIT_SIGN + stats.getNumberOfTimesPlayed() + SPLIT_SIGN
                + stats.getHighestScore() + SPLIT_SIGN + stats.getFastestTime() + GO_NEXT_LINE;
        try {
