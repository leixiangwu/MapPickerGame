...
public class RambleOnPanel extends JPanel {
    ...
    private void renderGamePlayScreen(Graphics g) {
        if (data.isGamePlayScreen()) {
            ...
            s = game.getGUIButtons().get(EXIT_GAME_TYPE);
            renderSprite(g, s);
    ...
    private void renderSubRegionsStack(Graphics g) {
        ...
        if (regionName != null) {
            ...
            Sprite sprite = game.getGUIDecor().get(STACK_BOX_TYPE);
            renderSprite(g, sprite);
            g.setColor(REGION_NAME_COLOR);
            g.setFont(font);
            g.drawString(textToRender, REGION_NAME_X, REGION_NAME_Y);
        }
    }
    ...
    private void renderFlagStack(Graphics g) {
        ...
        String textToRender = regionName + " Subregions' Flags";
        Font font = getMaxFitFont(g, textToRender, 280, 40, "Serif", Font.BOLD, 40);
        int counter = 0;
    ...
    private void renderLeaderStack(Graphics g) {
        String regionName = data.getRegionDataManager().getRegionName();
        Iterator<RegionSprite> it = data.getSubRegionStack().iterator();
        String textToRender = regionName + " Subregions' Leaders";
        Font font = getMaxFitFont(g, textToRender, 280, 40, "Serif", Font.BOLD, 40);
        int counter = 0;
        String leaderName;
    ...
    private void renderCapitalStack(Graphics g) {
        ...
        if (regionName != null) {
            LinkedList<RegionSprite> list = data.getSubRegionStack();
            Iterator<RegionSprite> it = list.iterator();
            String textToRender = regionName + " Subregions' Capitals";
            Font font = getMaxFitFont(g, textToRender, 280, 40, "Serif", Font.BOLD, 40);
            int counter = 0;
            while (it.hasNext() && (counter < 8)) {
                RegionSprite ts = it.next();
                regionName = ts.getRegionName();
                capital = data.getRegionDataManager().getCapitalMappedToSubRegion(regionName);
                int x = (int) (ts.getX());
                int y = (int) (ts.getY());
                int width = 300;
                int height = 50;
    ...
    public void renderGameEndScreen(Graphics g) {
        if (data.isGameEndScreen()) {
            ...
            g.drawString("Mode: " + data.getMode(), WIN_STATS_X, WIN_STATS_START_Y);
            g.drawString("Region: " + data.getRegionDataManager().getRegionName(), WIN_STATS_X, WIN_STATS_START_Y + WIN_STATS_Y_INC);
            g.drawString("Score: " + data.calculateScore(), WIN_STATS_X, WIN_STATS_START_Y + (WIN_STATS_Y_INC * 2));
            g.drawString("Game Duration: " + data.getGameWinDurationText(), WIN_STATS_X, WIN_STATS_START_Y + (WIN_STATS_Y_INC * 3));
            g.drawString("Sub Regions: " + data.getRegionDataManager().getNumberOfSubRegions(), WIN_STATS_X, WIN_STATS_START_Y + (WIN_STATS_Y_INC * 4));
            g.drawString("Incorrect Guesses: " + data.getIncorrectGuesses(), WIN_STATS_X, WIN_STATS_START_Y + (WIN_STATS_Y_INC * 5));
        }
    }
