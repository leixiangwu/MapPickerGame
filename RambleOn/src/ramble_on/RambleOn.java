...
public class RambleOn extends MiniGame {
    ...
    private RambleOnEventRelayer roer;
    ...
    private void initGamePlayScreenGUIControls() {
        ...
        sT = new SpriteType(STACK_BOX_TYPE);
        img = loadImage(STACK_BOX_PATH);
        sT.addState(DEFAULT_STATE, img);
        x = data.getGameWidth() - img.getWidth();
        y = 250;
        vX = 0;
        ...
        sT.addState(ENABLED_STATE, img);
        img = loadImage(LEFT_DISABLED_ARROR_PATH);
        sT.addState(DISABLED_STATE, img);
        x = 1100;
        y = 216;
        vX = 0;
        ...
        sT = new SpriteType(EXIT_GAME_TYPE);
        img = loadImage(EXIT_GAME_BUTTON_PATH);
        sT.addState(ENABLED_STATE, img);
        x = 1135;
        y = 214;
        vX = 0;
        vY = 0;
        s = new Sprite(sT, x, y, vX, vY, ENABLED_STATE);
        guiButtons.put(EXIT_GAME_TYPE, s);
        
        //TO HOLD THE END POSITION OF WINNING IMAGE POSITION
        int tempX;
        int tempY;
        ...
        vX = 0;
        vY = 0;
        img = loadImage(EXIT_BUTTON_PATH);
        x = tempX - img.getWidth();
        sT.addState(VISIBLE_STATE, img);
        img = new BufferedImage(1, 1, BufferedImage.TYPE_4BYTE_ABGR);
        sT.addState(INVISIBLE_STATE, img);
        s = new Sprite(sT, x, tempY, vX, vY, INVISIBLE_STATE);
    ...
    public DefaultTreeModel getWorldTreeModel() {
        return worldTreeModel;
    }
    ...
    public void initWorldTree() {
        ...
        String[] lineData;
        Vector<String> regionList = rambleOnDataModel.getAccountManager().getCurrentAccount().getRegionsPlayedList();
        for (String regionName : regionList) {
            if (!regionName.equalsIgnoreCase("The World")) {
                lineData = regionName.split("_");
                tempNode = getRegionNode(lineData[1]);
                if (lineData.length > 2) {
                    if (tempNode == null) {
                        addSubRegionsToTree(lineData[1], root);
                        addSubRegionsToTree(lineData[2], getRegionNode(lineData[1]));
                    } else {
                        addSubRegionsToTree(lineData[2], tempNode);
                    }
                } else {
                    if (tempNode == null) {
                        addSubRegionsToTree(lineData[1], root);
    ...
    private void initAudio() {
        ...
        try {
            ...
            audio.play(BACKGROUND_MUSIC, false);

            audio.loadAudio(STANDARD_VICTORY_SONG, STANDARD_VICTORY_SONG_FILE_NAME);
    ...
    public void addRambleOnEventRelayer() {
        ...
        roer = new RambleOnEventRelayer();
        canvas.addMouseListener(roer);
    }

    public void removeRambleOnEventRelayer() {
        canvas.removeMouseListener(roer);
    }

    public void initCurrentAccountHandlers() {
        GoForwardHandler gfh = new GoForwardHandler(rambleOnDataModel, this);
